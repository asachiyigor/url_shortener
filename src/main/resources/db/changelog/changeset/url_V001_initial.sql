...
(
    ...
    value      VARCHAR(7) COLLATE "C" NOT NULL,
    status     VARCHAR(10)            NOT NULL DEFAULT 'FREE',
    created_at TIMESTAMP WITH TIME ZONE        DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT hash_value_unique UNIQUE (value),
    CONSTRAINT hash_status_check CHECK (status IN ('FREE', 'RESERVED', 'USED'))
);

CREATE INDEX idx_hash_status ON hash (status);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_url_original_url_unique ON url (original_url);
...
INSERT INTO url_config (config_key, config_value, description)
VALUES ('default_expiration_days', '30', 'Default number of days before URL expiration'),
